...
int main()
{
	// Hide console window - winuser.h
	ShowWindow(GetConsoleWindow(), 0);
	// Hide Mouse Cursor - winuser.h
	ShowCursor(false);
	...
	// Variables for the screen dimension
	...
	// Varibales for background image
	...
	while (window.isOpen())
	{
		...
		for (uint i = 0; i < playerObj.bullets.size(); i++)
		{
			...
			for (uint j = 0; j < asteroids.size(); j++) // uint - because vector objects are unsigned
			{
				...
				{
					try
					// try - to eliminate the possibility of game crash due to collision between
					// bullet, asteroid and enemy ship, all at the same time
					{
						playerObj.bullets.erase(playerObj.bullets.begin() + i);
